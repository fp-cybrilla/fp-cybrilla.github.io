...
| ---------- | ------------ |
| **Redemption details** | |
| Cost | To calculate the total cost of purchases associated with a  transaction, <br>Let redeem_NAV = `traded_at`.<br>Let total_cost=0.<br>1. Iterate through each of the purchase `sources`.<br> 2. total_cost = total_cost + [(redeem_NAV * `sources[i].units`)-`sources[i].gain`] <br> Example:<br> redeem_NAV=Rs 15<br>total_cost=0.<br> Consider T3, 150 units have been redeemed. Out of 150 units sold, 100 units were purchased via T1 and 50 units were purchased via T2. So, sources are T1 and T2. <br>total_cost = 0 + (15*100-500) = 1000<br>total_cost = 1000 + (15 * 50-150) = 1600|
|Average cost|Average cost = Cost/Redeemed units<br>Average cost = 1600/150 = 10.67|
|Realized gains|Sum of `gain` of purchase sources of a transaction<br>Note: To determine whether a particular gain is a short term gain or a long term gain, please use `sources[i].days_held` attribute. For example, in case of equity funds, if days held > 365 days, the gain can be treated as a long term gain.|
| **Holdings** |
...
| Invested value amount | Let invested_value=0<br><br> 1. Iterate through each transaction <br><br>2. If transaction `type` is `purchase` or `switch_in`,<br>invested_value=invested_value+`amount`<br><br>3.If transaction `type` is `redemption` or `switch_out`, <br>invested_value=invested_value - cost of sources.<br><br>Note: Please refer "Cost" calculation described in "Redemption details" section to calculate  the cost of the sources. <br><br>Example:<br>After T1, invested_value would be 100 * 10 = Rs.1000<br>After T2, invested_value would be Rs. 1000+ 100*12 = Rs.2200<br>After T3, invested_value would be Rs2200- total cost of purchases.<br>Total cost of purchases = 1000+600 = Rs. 1600. (Please refer Cost section example)<br>After T3, invested_value would be Rs 2200 - 1600 = Rs. 600 |
