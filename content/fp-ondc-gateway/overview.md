## FP-ONDC Gateway Overview

The **FP-ONDC Gateway** enables you to process mutual fund orders via ONDC as a BuyerApp, using **Cybrilla** as the ONDC-verified TSP. This document highlights how the ONDC route differs from the traditional RTA route and outlines the key steps involved in onboarding and transaction flows.
...
* Address details
   * The `nature` parameter is mandatory when creating an address in ONDC.
...
To create the purchase orders, MF Investment Account must be created using [FPDocs, MF Investment Account](https://fintechprimitives.com/docs/api/#create-an-mf-investment-account). Please note the following restrictions for `ONDC` gateway:
...
2. **Review Completion:** The `ONDC` gateway will asynchronously review the order details in the background. If the review passes, the order is marked as pending and `mf_purchase.review_completed` event is delivered. Else, the order is marked as `failed`.
...
5. **Order Submission:** Next mark the order as `confirmed` using [FPDocs, Update MF Purchase](https://fintechprimitives.com/docs/api/#update-a-mf-purchase). Once confirmed, the order is submitted to the `ONDC` gateway and upon successful submission, its status becomes `submitted`.
...
> Redemptions via ONDC Gateway are supported **only** for folios created using the ONDC route.
...
2. **Review Completion:** The `ONDC` gateway will asynchronously review the order details in the background. If this review passes, the order is marked as `pending`  and `mf_redemption.review_completed` event is delivered. Else, the order is marked as `failed` .
...
4. **Order Submission:** When the order is `confirmed` , it will be submitted to the `ONDC`  gateway and upon successful submission, the order is marked as `submitted` . Upon the order processing at the RTA, the order moves to its final state of either `successful` or `failed` .
