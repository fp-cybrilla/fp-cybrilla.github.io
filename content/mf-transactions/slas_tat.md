...
**1.1 For orders routed via FP in ONDC `gateway`s**
...
**2.1 Payment collection and settlement against orders routed via ONDC `gateway`**
...
Irrespective of the order gateway, for complete order execution, 
...
In ONDC `gateway`,  AMC is responsible for settling the redemption proceeds to the bank account of investors.  At present, the payment cycle by AMCs for redemptions submitted before cut-off time are
 - T+1 for debt
